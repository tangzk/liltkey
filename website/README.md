...
v0.5.1 默认开启 SenseVoice 定稿校正，中英文首页只保留一句更新提示和详情入口，6 款模型的对比表、方案取舍和测试限制放在独立页面 `/research/`、`/zh/research/`。旧 `#research` 链接仍能定位到首页入口。强调功能默认开启、安装自动准备模型、保留明确关闭的个人设置，区分整句计算与松键后等待时间。版本、下载链接与根 README 和发布包同步。
...
- `dist/research/index.html`、`dist/zh/research/index.html`：英文、中文模型对比详情页，共用主站样式，提供返回首页及语言切换。
